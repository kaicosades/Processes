...
namespace Car
{
    internal class Car
    {
        ...
        {
            engine.Start();
            threads.EngineIdleThread = new Thread(EngineIdle); //создание потока
            threads.EngineIdleThread.Start(); //запуск потока
        }
                ...
                switch (key)
                {
                    ...
                    case ConsoleKey.F:
                        if (!driver_inside)
                        {
                            ...
                            //double amount = Convert.ToDouble(Console.ReadLine().Replace('.', ','));//replace вызывается, т к не статический метод
                            tank.Fill(amount);
                        }
                        else
                        {
