...
namespace InterprocessCommunication
{
    ...
    public partial class Form1 : Form
    {
        ...
        //объявления списка где храним процессы
        List<Process> processes = new List<Process>();
        int count = 0;//счетчик процессов
        ...
        public Form1()
        {
            ...
            buttonCloseWindow.Enabled = false;// неактивация кнопки
        }
        ...
        {
            Process proc = Process.Start(AssemblyName);// после старт можно поставить расширение (assemblyName.Split('.')[0])
                                                       //Process proc = new Process();
                                                       //proc.StartInfo = new ProcessStartInf(assemblyName);
                                                       //proc.Start();
            processes.Add(proc);
        ...
        void Proc_Exited(object sender, EventArgs e)
        {
            ...
            Process proc = sender as Process;//преобразование сендера в процесс
        ...
        {
            int parentId = 0;
            using (System.Management.ManagementObject obj = new ManagementObject($"win32_process.handle={id}"))//add referens system management в первой вкладке
            {
                obj.Get();
        ...
        delegate void ProcessDelegate(Process proc);// указатель на функцию
